...
public class MessageConsumer implements MetaInfoApi {
    ...
    @KafkaListener(id = "data-processor-user-save-listener", topicPartitions = @TopicPartition(topic = KafkaTopics.CrudMeta.SAVE_USER_META_INFO_TOPIC, partitions = {"0","1"}), groupId = "save-meta-group")
    @Override
    ...
    @KafkaListener(id = "data-processor-user-get-ext-listener", topicPartitions = @TopicPartition(topic = KafkaTopics.CrudMeta.GET_USER_EXT_META_INFO_TOPIC, partitions = {"0","1"}), groupId = "get-ext-meta-group")
    @Override
    public ExternalMetaInfoDto getExternalMeta(ArtifactMetadataGetRequest request) throws DataNotFoundException {
    ...
    @KafkaListener(id = "data-process-user-get-int-listener", topicPartitions = @TopicPartition(topic = KafkaTopics.CrudMeta.GET_USER_INT_META_INFO_TOPIC, partitions = {"0","1"}), groupId = "get-int-meta-group")
    @Override
    ...
    @KafkaListener(id = "data-process-user-get-all-ext-listener", topicPartitions = @TopicPartition(topic = KafkaTopics.CrudMeta.GET_USER_ALL_EXT_META_INFO, partitions = {"0","1"}), groupId = "get-all-ext-meta-group")
    @Override
    ...
    @KafkaListener(id = "data-processor-user-del-listener", topicPartitions = @TopicPartition(topic = KafkaTopics.CrudMeta.DEL_USER_META_INFO, partitions = {"0","1"}), groupId = "del-meta-group")
    @Override
    public boolean delete(ArtifactMetadataGetRequest request) {
        logger.info("Received request to delete metadata for userId: {}", request.getUserId());
        String messageKey = MessageKeyHolder.get();
        boolean result = metaInfoService.deleteMetaInfo(request);
        if (result) {
            logger.info("Metadata deleted successfully for artifactId: {}", request.getArtifactId());
        } else {
